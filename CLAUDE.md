...
- `src/klipperctl/tui/` — Interactive TUI (optional, requires `textual`):
  ...
  - `screens/console.py` — Full-screen GCode console; embeds a `DashboardConsoleWidget` (constructed with `release_focus_on_escape=False` so a single Escape press pops the screen, and an optional `MessageFilter`) and stretches it via screen-level CSS to fill the area below the header. All backfill, live-tail polling, dedupe, and history-recall behavior comes from the embedded widget — no inline RichLog/Input.
  ...
  - `widgets/dashboard_console.py` — `DashboardConsoleWidget` used by both the dashboard embed and the full `ConsoleScreen`. Composes a `RichLog` + `Input`, maintains a 50-command history deque cycled via Up/Down, posts a `DashboardConsoleWidget.Submitted` message on Enter, and receives replies via a callback path from `KlipperApp.send_gcode(on_result=...)`. On mount, the widget backfills the log with the last 25 entries from `KlipperApp.fetch_gcode_store` so the user sees recent printer activity on launch (command entries render in dim cyan, responses in plain dim, each prefixed with a timestamp from `klipperctl.output.format_timestamp`), then starts a **live-tail poll loop** via `set_interval(tail_interval, ...)` that calls `fetch_gcode_store` every ~2s and renders any entries newer than a `_last_time` watermark (seeded from the backfill). The live path dedupes the user's own just-submitted commands against a bounded `_local_echo_history` deque — command entries matching a recent submission are skipped, and response entries within 3s of a submission are assumed to be that submission's reply (already rendered by the `on_result` callback). Live entries render in normal colors (bold cyan for commands, green for responses) to stay visually distinct from historical backfill. Two configuration knobs let it serve both use cases: `release_focus_on_escape=True` (the dashboard embed default) makes Escape release focus from the input via `on_key`+`event.stop()` so the dashboard's single-key bindings work again without the screen's quit binding firing; `release_focus_on_escape=False` (the full ConsoleScreen) lets Escape bubble so a single press pops the screen. An optional `msg_filter: MessageFilter` is applied to both backfill and live-tail entries — entries whose messages don't match are skipped — so the full ConsoleScreen can honor its `MessageFilter` constructor arg.
